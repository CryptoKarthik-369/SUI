...
// TradingView REQUIRES port 80 (HTTP) or 443 (HTTPS) for webhooks
// Local: sudo node server.js (port 80 needs root on Linux/Mac)
// Deploy to Railway/Render/Fly.io — they handle port mapping automatically
const PORT = process.env.PORT || 80;
const WS_PORT = process.env.WS_PORT || PORT;  // WS runs on same port
...
server.listen(PORT, () => {
    ...
    console.log(`  Webhook endpoint    →  POST http://YOUR_IP:${PORT}/webhook`);
    console.log(`  WebSocket           →  ws://YOUR_IP:${PORT}/ws`);
    console.log(`  API state           →  GET  http://YOUR_IP:${PORT}/api/state`);
    console.log("───────────────────────────────────────────────────────");
    console.log("  TradingView NEEDS port 80 or 443!");
    console.log("  → Deploy to Railway.app / Render.com  (easiest, free)");
    console.log("  → OR use ngrok:  ngrok http " + PORT + "  then paste ngrok URL");
    console.log("  → OR VPS: use nginx reverse proxy on port 80 → " + PORT);
    console.log("═══════════════════════════════════════════════════════");
});
